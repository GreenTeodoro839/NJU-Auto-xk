...
3. 安装Python **3.12** （或以下），并安装依赖：
   ...
   ```
   pip install requests urllib3 pycryptodome serverchan-sdk pillow numpy ultralytics openai # 如果要用代理还需要下面这个 pip install "requests[socks]"
   ```
...
5. 修改xk.conf 请按照说明修改模板，不需要的地方**把中文删掉留空**
...
8. 修改course.conf
   ...
   ```
   { "electiveBatchCode": "正常情况下这里已经填好了，不要动", "courses": [ ["ClassID1", "KIND1", "TYPE1"], ["ClassID2", "KIND2", "TYPE2"] ] }
   ```
