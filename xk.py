...
def load_course_conf() -> Tuple[str, List[Tuple[str, str, str]]]:
    ...
    course.conf 必须是 JSON 对象：
    {
      "electiveBatchCode": "...",
      "courses": [
        ["teachingClassId", "courseKind", "teachingClassType"],
        ...
      ]
    }
    ...
    for i, item in enumerate(raw_courses):
        if not (isinstance(item, (list, tuple)) and len(item) == 3):
            raise ValueError(
                f"course.conf courses 第 {i+1} 项必须是 [teachingClassId, courseKind, teachingClassType]，实际: {item}"
            )
        ...
        courses.append((str(class_id), str(kind), str(ctype)))

    return elective_batch_code, courses


def save_course_conf(elective_batch_code: str, courses: List[Tuple[str, str, str]]) -> None:
    """保存 course.conf（新格式）。"""
    data = {
        "electiveBatchCode": str(elective_batch_code).strip(),
        "courses": [[c[0], c[1], c[2]] for c in courses],
    }
    _save_json_atomic(COURSE_FILE, data)


def load_courses() -> List[Tuple[str, str, str]]:
    """仅返回课程列表（用于兼容脚本内部旧调用）。"""
    _, courses = load_course_conf()
    return courses


def remove_course_from_file(course: Tuple[str, str, str]) -> bool:
    """从 course.conf 删除某门课。"""
    try:
...
def main() -> None:
    ...
    while True:
        ...
        for idx, course in enumerate(list(courses), 1):
            class_id, kind, ctype = course
            print(f"\n[{idx}/{len(courses)}] 班级ID={class_id}, courseKind={kind}, teachingClassType={ctype}")
